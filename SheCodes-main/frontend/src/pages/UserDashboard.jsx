...
import { getUser, getMyResults, getDoctors, getRhythmRecallRecommendation } from "../services/api";
...
export default function UserDashboard({ setPage }) {
  ...
  useEffect(() => {
    ...
    getRhythmRecallRecommendation()
      .then(rec => setRhythmRec(rec))
      .catch(() => {});
  }, []);
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <>
          {/* ── Evening Sundowning / Melodic Wind-down Routine Banner ── */}
          {(rhythmRec?.recommended || (new Date().getHours() >= 16 && new Date().getHours() <= 20)) && (
            <div
              style={{
                marginBottom: 24,
            ...
            >
              <div style={{ flex: 1, minWidth: 260 }}>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(245,158,11,0.2)", color: AMB, padding: "3px 10px", borderRadius: 999, fontSize: 11, fontWeight: 800, textTransform: "uppercase", marginBottom: 6 }}>
                  <span>🌅</span> Evening Sundowning Routine
                </div>
                <h3 style={{ margin: "2px 0 6px", fontSize: 18, fontWeight: 900, color: "#fff" }}>
                  Rhythm & Recall: Melodic Agitation Wind-Down
                </h3>
                <p style={{ margin: 0, fontSize: 13, color: "#cbd5e1", lineHeight: 1.5, maxWidth: 620 }}>
                  {rhythmRec?.calming_prompt || "Late afternoon and evening hours can trigger restlessness (sundowning). Music stimulates preserved procedural memory circuits, soothing agitation and bringing gentle comfort."}
                </p>
              </div>
              <button
                onClick={() => setPage("game-rhythm")}
                style={{
                  minHeight: 52,
                  padding: "0 24px",
                  borderRadius: 14,
