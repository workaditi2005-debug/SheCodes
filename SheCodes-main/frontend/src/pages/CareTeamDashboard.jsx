...
export default function CareTeamDashboard({ doctor = false }) {
  ...
  const select = useCallback(async (patient) => {
    try {
      ...
      {detail && (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 20, marginTop: 24 }}>
            ...
            {/* Rhythm & Recall / Sundowning Engagement Card */}
            <Card
              title="Evening Melodic Engagement & Sundowning Log"
              icon="🎵"
              badge={{ text: "Preserved Musical Memory", style: { background: "rgba(236,72,153,0.15)", color: "#f472b6" } }}
              style={{ gridColumn: "span 2" }}
            >
              {(() => {
                const rhythmSessions = (detail.game_activity || []).filter(s => s.game_id === "rhythm_and_recall");
                if (rhythmSessions.length === 0) {
                  return (
                    <div style={{ padding: "16px", background: "rgba(255,255,255,0.02)", borderRadius: 12, color: "#94a3b8", fontSize: 13 }}>
                      No evening music sessions completed yet. Music engagement stimulates preserved caudal anterior cingulate circuits to reduce sundowning agitation.
                    </div>
                  );
                }

                const avgAccuracy = Math.round(rhythmSessions.reduce((acc, s) => acc + (s.telemetry?.rhythm_accuracy || s.score || 85), 0) / rhythmSessions.length);
                const positiveShiftCount = rhythmSessions.filter(s => (s.telemetry?.mood_shift || 0) > 0).length;

                return (
                  <div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: 16 }}>
                      <div style={{ padding: "12px 14px", background: "rgba(255,255,255,0.03)", borderRadius: 12 }}>
                        <div style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase", fontWeight: 700 }}>Total Sessions</div>
                        <div style={{ fontSize: 24, fontWeight: 900, color: "#f8fafc", marginTop: 4 }}>{rhythmSessions.length}</div>
                      </div>
                      <div style={{ padding: "12px 14px", background: "rgba(255,255,255,0.03)", borderRadius: 12 }}>
                        <div style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase", fontWeight: 700 }}>Rhythm Accuracy</div>
                        <div style={{ fontSize: 24, fontWeight: 900, color: "#14b8a6", marginTop: 4 }}>{avgAccuracy}%</div>
                      </div>
                      <div style={{ padding: "12px 14px", background: "rgba(255,255,255,0.03)", borderRadius: 12 }}>
                        <div style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase", fontWeight: 700 }}>Agitation Relief</div>
                        <div style={{ fontSize: 24, fontWeight: 900, color: "#f59e0b", marginTop: 4 }}>
                          {positiveShiftCount} / {rhythmSessions.length} Calm
                        </div>
                      </div>
                    </div>

                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {rhythmSessions.slice(-3).reverse().map((s, idx) => (
                        <div
                        ...
                        >
                          <div>
                            <strong style={{ color: "#f8fafc" }}>{s.telemetry?.song_title || "Nostalgic Melody"}</strong>
                            <span style={{ color: "#64748b", marginLeft: 8 }}>
                              ({s.telemetry?.mode === "free_sing" ? "Hum & Sing" : s.telemetry?.mode === "song_recognition" ? "Recognition" : "Beat Tap"})
                            </span>
                          </div>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <span style={{ color: "#14b8a6", fontWeight: 700 }}>{s.score}% Engagement</span>
                            {(s.telemetry?.mood_shift ?? 1) >= 0 && (
                              <span style={{ background: "rgba(20,184,166,0.15)", color: "#2dd4bf", padding: "2px 8px", borderRadius: 999, fontSize: 11, fontWeight: 700 }}>
                                +{s.telemetry?.mood_shift || 1} Calmer 😊
                              </span>
                            )}
                          </div>
                        </div>
